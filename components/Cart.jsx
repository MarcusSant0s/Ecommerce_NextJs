...
const Cart = () => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-full bg-gray-800 bg-opacity-50 transition-transform ${showCart ? 'translate-x-0' : 'translate-x-full'}`}
    ...
    >
      <div className='h-full w-full flex md:justify-end'>
        <div className='w-full bg-white flex flex-col md:flex-end md:w-3/4' ref={cartRef}>
          ...
          <button 
            type='button' 
            className='flex mt-5 mb-5 shadow-md shadow-black/20 w-full pb-2'
            onClick={() => setShowCart(false)}
          >
            <AiOutlineLeft className='text-4xl ml-5' />
            <span className='self-center text-lg'>Seu carrinho</span>
            <span className='self-center ml-1 text-md'>({totalQuantities} itens)</span>
          </button>
          ...
          <div className='bg-white'>
            {cartItems.length >= 1 && cartItems.map((item) => (
              <div key={item._id} className="flex items-center gap-4 p-4 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow">
                <Image
